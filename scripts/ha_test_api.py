...
def _validated_base_url(base_url: str) -> str:
    """Validate script base URL to avoid unexpected URL schemes."""
    parsed = urllib.parse.urlsplit(base_url)
    if parsed.scheme not in {"http", "https"}:
        raise ValueError("HA_TEST_URL must use http or https")
    if (
        parsed.username
        or parsed.password
        or parsed.query
        ...
        raise ValueError("HA_TEST_URL must be a plain http/https origin URL")
    if not parsed.hostname:
        raise ValueError("HA_TEST_URL must include a valid hostname")
    if parsed.port:
...
class HomeAssistantApi:
    ...
    def __init__(
        ...
        *,
        ...
        self.base_url = _validated_base_url(base_url).rstrip("/")
