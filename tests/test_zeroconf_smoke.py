...
class TestZeroconfSmoke(unittest.TestCase):
    ...
    def test_run_smoke_without_dependency(self) -> None:
        with (
            patch("scripts.zeroconf_smoke.Zeroconf", None),
            patch("scripts.zeroconf_smoke.ServiceBrowser", None),
        ):
            result = zeroconf_smoke.run_zeroconf_smoke(
                service_type="_ste-dhe._tcp.local.",
                timeout=0.01,
        ...
        self.assertFalse(result.ok)
        self.assertIn("zeroconf package is not installed", result.message)
