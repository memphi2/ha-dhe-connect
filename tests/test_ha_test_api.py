...
def _fixture_backup_path(name: str) -> Path:
...
class TestHATestApi(unittest.TestCase):
    ...
    def test_cleanup_retry_repeats_until_auth_file_stays_clean(self) -> None:
        backup_path = _fixture_backup_path("auth-backup.json")
        results = [
            ha_test_api.TokenCleanupResult(removed=70, backup_path=backup_path),
            ha_test_api.TokenCleanupResult(removed=1, backup_path=_fixture_backup_path("again.json")),
    ...
    def test_validated_base_url_rejects_unsupported_scheme(self) -> None:
        with self.assertRaises(ValueError):
            ha_test_api.HomeAssistantApi("ftp://ha.test")

    def test_validated_base_url_rejects_non_origin_inputs(self) -> None:
        for base_url in (
            "http://user:pass@ha.test",
            "http://ha.test:8123/status",
            "http://ha.test/path?x=1",
            "https://ha.test#frag",
        ):
            with self.subTest(base_url=base_url), self.assertRaises(ValueError):
                ha_test_api.HomeAssistantApi(base_url)

    def test_validated_base_url_normalizes_host_port(self) -> None:
        api = ha_test_api.HomeAssistantApi("https://HA.TEST:8123")
        self.assertEqual(api.base_url, "https://ha.test:8123")
