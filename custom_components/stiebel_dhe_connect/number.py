"""Number platform for DHE Connect."""
...
class StiebelDHENumber(StiebelDHEEntityMixin, RestoreNumber):
    ...
    def _number_available(self) -> bool:
        """Return whether this writable number can currently be used."""
        if self.entity_description.temperature_memory_slot is not None:
            return bool(self._client.available)
        return value_available(self._client.available, self._attr_native_value)
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self._attr_available = self._number_available()
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        try:
            ...
            raise dhe_action_error(
                f"Could not set DHE number {self.entity_description.key}",
                err,
            ) from err
