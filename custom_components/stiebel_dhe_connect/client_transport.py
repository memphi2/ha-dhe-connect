...
WEBSOCKET_IDLE_TIMEOUT_MIN = 35.0
...
def _websocket_receive_timeout(ctx: DHESession) -> float:
    """Return the maximum time to wait for any WebSocket packet."""
    return max(
        WEBSOCKET_IDLE_TIMEOUT_MIN,
        ctx.ping_interval * WEBSOCKET_IDLE_TIMEOUT_FACTOR,
    )
...
class DHEClientTransportMixin(
    ...
    def _record_session_connected(self) -> None:
        was_reconnect = self._has_connected
        self._connection_supervisor.mark_connected(count_reconnect=was_reconnect)
    ...
    async def _upgrade_to_websocket(self, ctx: DHESession) -> None:
        ...
        try:
                ...
                try:
                    ...
                    while time.monotonic() < deadline:
                        ...
                        if packet == "3" or not packet:
                            continue
                        _LOGGER.debug(
                            "Ignoring unexpected DHE websocket probe packet: %s",
                            _summarize_diagnostic_value(packet),
                        )
            ...
            self._record_websocket_upgrade_failure()
            if websocket is not None and not websocket.closed:
                await websocket.close()
    ...
    def _record_websocket_upgrade_failure(self) -> None:
        """Track failed WebSocket upgrades for support diagnostics."""
        self._websocket_upgrade_failures += 1
    ...
    async def _read_websocket_events_once(self, ctx: DHESession) -> list[DHEEvent]:
        ...
        if websocket is None:
            return []
        receive_timeout = _websocket_receive_timeout(ctx)
        try:
            ...
            return [
                DHEEvent(
                    "__closed",
                    f"DHE websocket idle timeout after {receive_timeout:.1f}s",
                )
            ]
        if message.type in {
            aiohttp.WSMsgType.CLOSE,
