...
def _normalize_diagnostic_key(key: str) -> str:
    """Normalize mixed-case and punctuation-heavy keys for key matching."""
    snake_case = re.sub(r"(?<!^)(?=[A-Z])", "_", key)
    return re.sub(r"[^a-z0-9_]+", "_", snake_case.lower()).strip("_")
...
def summarize_diagnostic_value(value: Any, *, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        ...
        summary = {
            _unique_redacted_key(key, key_counts): (
                "<redacted>"
                if _is_diagnostic_sensitive_key(key)
                else summarize_diagnostic_value(value[key], depth=depth + 1)
            )
            for key in keys
        }
...
def _is_diagnostic_sensitive_key(key: Any) -> bool:
    ...
    if normalized in _DIAGNOSTIC_SENSITIVE_KEYS:
        return True
    if "token" in parts:
        return True
    return any(part in _DIAGNOSTIC_SENSITIVE_KEYS for part in parts)
