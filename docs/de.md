...
- [Geraete- und Firmware-Matrix](firmware_matrix.md)
...
Discovery-Funde werden temporaer in Home Assistant gespeichert. Der Cache
erstellt kein Geraet und ersetzt kein Pairing; er reduziert wiederholte
Zeroconf-Prompts nach Neustarts, macht kuerzlich gefundene Ziele im manuell
gestarteten Setup wieder auswaehlbar und liefert anonymisierte Discovery-
Diagnosedaten. Erfasst werden Quelle, erste/letzte Sichtung, Confidence,
bevorzugte Identitaetsquelle und widerspruechliche Identitaetshinweise. Fuer
kurze Debug-Laeufe kann `DHE_CONNECT_DISCOVERY_DEBUG=1` gesetzt werden.
Die Diagnosen fassen ausserdem Cache-Status, Cache-Alter, Zeroconf-Alter und
Prompt-Unterdrueckung zusammen, ohne Hosts, IP-Adressen oder Service-Namen
auszugeben.
...
Diagnose-Textfeld geaendert werden; der DHE erlaubt dabei maximal 30 Zeichen.
...
Wellness-Programmschalter behalten stabile Entity-IDs. Name und Metadaten der
Programme werden aber aus dem DHE-Katalog `ste.app.wellness:programs`
uebernommen, sobald der DHE ihn sendet. Das Attribut `coldwater=true` bedeutet,
dass das Programm eine Kaltwasserphase enthaelt, in der das Geraet die Heizung
selbst abschaltet.
...
einem Geraetehersteller, Home Assistant oder HACS verbunden, nicht von diesen
unterstuetzt, gesponsert oder anderweitig freigegeben.
...
Hersteller-JavaScript, CSS, HTML, Firmware, Screenshots und Produktgrafiken
werden bewusst nicht mitgeliefert. Die Protokollnotizen beschreiben nur
beobachtetes Interoperabilitaetsverhalten und ersetzen keine
Hersteller-Software, Hersteller-Dokumentation oder Hersteller-Spezifikation.

Die englische Richtlinie zur Lizenz-, Marken- und Asset-Hygiene steht in
