...
| Wellness runtime | `s` | `duration`, diagnostic, disabled by default | `measurement` | ODB ID `32` (`ODB_Wellness_Zeit_Norm`) observed as a runtime seconds counter while the program/water flow is active and reset to `0` when it stops; when connected without a cached runtime value it starts at `0` |
