...
`set:ste.common.odb:value` or `assign:ste.common.odb:value` messages. Requested
readbacks are treated as fresh device state and are forwarded to measurement
entities even when the value matches the client cache, except for the zero
